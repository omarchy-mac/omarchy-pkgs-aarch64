...
The verifier checks source revisions, contents, actual add-on dependencies and unique file ownership, then runs real pacman fresh, full-baseline upgrade and repeat transactions in scratch roots under fakeroot. It seeds the installed base package database so dependency checks stay enabled. Scriptlets and hooks are disabled to keep the host untouched. Those tests do not prove signed repository selection, install hooks, boot or physical hardware behavior.
