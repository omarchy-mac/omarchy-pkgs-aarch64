...
| `brave-origin-bin` | 1:1.94.121-1 | Minimalist browser from the Brave team |
...
These helpers do not resolve remote branches, schedule builds, qualify an ARM installation, or authorize publication. The future workflow must still bind the plan to the executed build/import inventory, verify the complete snapshot, require qualification, and recheck the baseline manifest under the publishing lock. No automatic edge job is enabled here.
