...
## Signing a selected development candidate

The separate `Sign quattro image inputs` manual workflow reuses the existing repository signing helper and the `package-signing` environment. That environment requires Naeem's approval. It accepts a successful main-branch candidate build run ID, its exact artifact name, the reviewed SHA256 of `manifest.json`, and the full desktop source revision. PR and fork builds are ineligible. Hourly builds remain unsigned and do not wait for signing approval.

The signer checks the three package hashes, embedded revisions, versions, dependencies, ownership, and package manifests before loading the signing subkey. It runs without network access, does not execute package scripts, and signs private copies without changing archive bytes. No release or repository database is created, no feed is updated, and existing users receive nothing from this workflow.

The resulting `signed-quattro-image-inputs-<source>-<signing run>-<attempt>` artifact lasts 30 days. It contains the three archives and detached signatures, the original build manifest and its signature, both package lists and their signatures, a signed `signing.json` receipt, and new checksums. The original manifest retains `signing: none` to describe the build output; `signing.json` records the subsequent signing operation and exact input manifest digest. Build logs and recipes remain in the original artifact.
