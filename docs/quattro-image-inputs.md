...
Every new successful main-branch candidate build selecting exactly `quattro-upstream` (or leaving the source ref empty to use that default) automatically runs `Sign three candidate packages`. It reuses the existing signing helper and credentials in `package-signing-edge`, whose main-branch policy has no per-run human approval. The environment name does not select a publication destination. This job has only `contents: read`, uploads Actions artifacts, and never runs the edge publisher. The separate manual signing workflow is removed.

The source revision, unsigned artifact name and exact manifest checksum pass directly between jobs in the same run. The signing job checks out the detected package-tooling revision, rechecks the three package hashes, embedded revisions, versions, dependencies, ownership and package manifests, then loads the signing subkey. Signing runs without network access, executes no package scripts, and changes no archive bytes. PR builds and manual builds of other source refs remain unsigned. Forced rebuilds of `quattro-upstream` still sign automatically. No release or repository database is created, no feed is updated, and existing users receive nothing from this workflow.

The resulting `signed-quattro-image-inputs-<source SHA>-<build-input digest>` artifact contains the three archives and detached signatures, the original build manifest and its signature, both package lists and their signatures, a signed `signing.json` receipt, and new checksums. The original manifest retains `signing: none` to describe the build output; `signing.json` records the subsequent signing operation and exact input manifest digest. Build logs and recipes remain in the unsigned artifact. Signing tooling and keyring changes are included in the build-input digest and trigger a new candidate set.
