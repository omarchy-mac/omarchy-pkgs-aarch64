...
Consumers must verify the signatures against the reviewed public key and fingerprints in `pkgbuilds/omarchy-mac-keyring/signing-policy.json`, then verify the receipt's file hashes and selected source revision. A public key supplied by the artifact is not a trust anchor. Signing authenticates the candidate; it does not establish installation or boot qualification. The image builder still needs that verification and dependency-installation integration.
