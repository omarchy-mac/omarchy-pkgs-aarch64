...
The scheduled source is the protected `omacom/omarchy-mac:quattro-upstream` branch. Manual dispatch can select another source ref; the current signer validates its recorded commit but does not enforce membership in that protected branch. Operators who can dispatch the workflow can therefore request signing of another source revision. The same approved CI subkey is used for candidates and edge, so artifact-only delivery is not a cryptographic restriction on where a signed package could later be installed.
