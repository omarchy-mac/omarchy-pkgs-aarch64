...
class CandidateTest(unittest.TestCase):
    def setUp(self):
        ...
        for name in s.build.PACKAGES:
            deps = {'omarchy': ['omarchy-settings=1.0', 'snapper'], 'omarchy-settings': [], 'omarchy-mac': ['omarchy'], 'libva-v4l2_request-avd': ['glibc', 'libva']}.get(name, [])
        ...
        self.data = dict(schema=4, package_repository_revision=self.source, candidate_only=True, publication='none', signing='none',
    ...
    def test_real_archives_and_embedded_manifests(self):
        self.assertEqual(len(self.validate()[1]), 11)

    def test_old_firmware_including_schema_is_rejected(self):
        self.data['schema'] = 3
        self.save()
        with self.assertRaisesRegex(ValueError, 'not an unsigned candidate'):
            self.validate()
